...
| :--- | :--- | :--- |
| **Carga de Entorno** | Modificado desde la **Celda 1** en los notebooks iniciales *(Librerías estándar / Sin cambios / Añade `multipletests`)* hasta la **Celda 2** en la versión final. | Profesionaliza el entorno ocultando advertencias de sintaxis y fijando una paleta homogénea (`Set2`). |
| **Control SRM (Muestra)** | No existente en versiones previas. Implementado a partir de la **Celda 4** *(Mapeo plano y Cálculo Chi² manual)* hasta consolidar en la **Celda 5** una validación robusta. | **Eliminación de sesgo:** Asegura matemáticamente que el ruteo de usuarios fue totalmente balanceado y limpio. |
| **Filtro de Contaminación** | Característica nueva: No existente en las primeras versiones. Añadido en **Celda 5** *(Cruce de IDs)* y optimizado en **Celda 6** *(Reporte de duplicados = 0)*. | **Garantía de Rigor:** Valida que ningún usuario haya sido expuesto a ambas variantes de manera simultánea. |
| **Efecto Novedad** | No existente en versiones previas. Incorporado en **Celda 6** *(Eje de tiempo vacío)* y expandido en **Celdas 7-8** con series diarias y análisis de estabilidad. | **Protección de Falsos Positivos:** Confirma que el éxito de B es estable y duradero, no una anomalía inicial. |
| **Análisis de Gasto** | Evoluciona de un espacio vacío a un *t-test* estándar (**Celda 5 y 6**), migrando a *Levene + t-Welch + MWU* (**Celda 10-11**) y culminando en **Celdas 11-13** *(Añade Cohen's d)*. | **Precisión Estadística:** Levene detectó varianzas distintas, obligando a cambiar a una robusta **t de Welch**. |
| **Control de Outliers** | No existente en las primeras tres versiones. Implementado en **Celda 12** *(Filtro estadístico IQR)* y perfeccionado en **Celdas 14-15** *(Comparativa de medias con/sin atípicos)*. | **Robustez del Negocio:** Demuestra que la mejora en gasto no se debe a compradores masivos anómalos aislados. |
| **Métrica Reina (RPV)** | No existente originalmente. Pasa de un cálculo simple en la **Celda 15** a una resolución formal de la paradoja en las **Celdas 16-18**. | **Traducción Financiera:** Unifica conversión y ticket promedio para reflejar un aumento directo del **+42.8%** por visita. |
| **Resolución del Error Crítico** | No existente en inicios. Corrige la caída por `KeyError: 'device'` (**Celda 15**), pasa por un parchado rudimentario (**Celda 18**) y finaliza con mapeo dinámico en **Celdas 19-23**. | **Estabilidad de Software:** Elimina referencias a columnas muertas inexistentes en el archivo físico del dataset. |
| **Reducción de Ruido** | No existente al inicio. Se aplicó en **Celda 24** *(V de Cramér / Bonferroni)* y posteriormente estas celdas fueron **Eliminadas por Diseño** en la versión final. | **Navaja de Ockham:** Remoción de sobre-ingeniería matemática que no aportaba valor a la toma de decisiones. |
| **Dashboard Ejecutivo** | Evoluciona desde celdas vacías y gráficos dispersos con `FutureWarning` hacia un diseño unificado en la **Celda 26** *(Exportación de panel unificado PNG)*. | **Estándar de Production-Ready:** Consolida métricas clave en una única imagen lista para el C-Level. |
---
